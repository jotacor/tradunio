...
def main():
    ...
    parser.add_argument('-c', '--comprar', action='store_true', dest='comprar', help='Muestra los jugadores que tienes que comprar.')
    parser.add_argument('-t', '--trans', action='store_true', dest='trans', help='Descarga de comunio las transacciones y las guarda en base de datos.')
    parser.add_argument('-u', '--update', action='store_true', dest='update', help='Actualiza todos los datos de usuario y sus jugadores en la base de datos.')
    ...
    if args.update or args.all:
        ...
        print 'Valor del equipo: %s € - Dinero: %s €' % (locale.format("%d", teamvalue, grouping=True), locale.format("%d", money, grouping=True))
    ...
    if args.comprar or args.all:
        ...
        for player in on_sale:
            ...
            owner = player[6]
            last_points = db.simple_query('SELECT p.points,g.gameday FROM players pl INNER JOIN points p ON p.idp=pl.idp INNER JOIN gamedays g ON p.idg=g.idg AND pl.name="%s" ORDER BY cast(g.gameday as unsigned) ASC' % name)[-5:]
...
def clean_players():
    ...
    for limpia in limpiar:
        borrar = db.simple_query('SELECT idp,date FROM transactions WHERE type="Buy" AND idp=? AND date<? ORDER BY date DESC LIMIT 1', (limpia[0],limpia[1],))
        if borrar:
...
def check_sell(my_players):
    ...
    for player in my_players:
        ...
        compra = db.simple_query('SELECT date,price FROM transactions WHERE idp=%s AND type="Buy" ORDER BY date DESC LIMIT 1' % idp)
        if compra == None:
            precio_actual = float(db.simple_query('SELECT price FROM prices WHERE idp=%s ORDER BY date DESC LIMIT 1' % idp)[0])
            precio_inicial = float(db.simple_query('SELECT price FROM prices WHERE idp=%s AND date>%s ORDER BY date ASC LIMIT 1' % (idp,'%s0801' % date.today().year))[0])
        ...
        barras = db.simple_query('SELECT idp,date,price FROM prices WHERE idp=%s AND date>=%s ORDER BY date ASC' % (idp, compra[0]))
        ...
        if vende:
            vender.append([idp,name,'%s%s%s'%(GREEN,vende,ENDC),fecha_compra,precio_compra,precio, colorize_rentability(rent),rent])
        else:
            vender.append([idp,name,'%s'%(vende),fecha_compra,precio_compra,precio, colorize_rentability(rent),rent])

        # Guardamos rentabilidad de los jugadores en cartera
        db.commit_query('INSERT IGNORE INTO rentabilities (idp, date, rentability) VALUES (%s, %s, %s)' % (idp, hoy, int(rent)))
...
def write_transactions(myid):
    ...
    for trans in from_you:
        if trans[6] == 'Efectuada':
            ...
            db.nocommit_query('INSERT OR REPLACE INTO transactions (idp,type,price,date) VALUES (?,?,?,?)', (idp,'Buy',price,trans_date,) )
            db.commit()
    ...
    for trans in to_you:
        if trans[6] == 'Efectuada':
            ...
            trans_date=translate_dates([trans[5]])[0]
            idp=com.info_player_id(player)
            db.commit_query('INSERT OR REPLACE INTO transactions (idp,type,price,date) VALUES (?,?,?,?)', (idp,'Sell',price,trans_date,) )
            compra=db.simple_query('SELECT date,price FROM transactions WHERE idp=? AND type="Buy" ORDER BY date DESC LIMIT 1', (idp,))
            if compra == None:
                #Si el jugador lo tenemos dede el inicio...
                compra=db.simple_query('SELECT date,price FROM prices WHERE idp=? AND date>? ORDER BY date ASC LIMIT 1', (idp,'%s0801' % date.today().year,))
                compra=c.fetchone()
            precio_compra=float(compra[1])
            ...
            #c.execute('DELETE FROM prices WHERE idp=?', (idp,) )
            #c.execute('DELETE FROM players WHERE idp=?', (idp,) )
        elif trans[6] == 'Pendiente':
            player=trans[0]
            price=trans[3]
            trans_date=translate_dates([trans[5]])[0]
            idp=com.info_player_id(player)
            c.execute('SELECT date,price FROM transactions WHERE idp=? AND type="Buy" ORDER BY date DESC LIMIT 1', (idp,))
            compra=c.fetchone()
            if compra == None:
                #Si el jugador lo tenemos dede el inicio...
                c.execute('SELECT date,price FROM prices WHERE idp=? AND date>? ORDER BY date ASC LIMIT 1', (idp,'%s0801' % date.today().year,))
                compra=c.fetchone()
            precio_compra=float(compra[1])
            #Calculamos la rentabilidad y configuramos el color
            rent=(price-precio_compra)/precio_compra*100
            ret.append([idp,player,float(price),trans_date,'%sOffer%s'%(YELLOW,ENDC), colorize_rentability(rent)])
...
def _days_wo_price(player):
    ...
    '''
    idp = com.info_player_id(player)
    c.execute('SELECT MAX(date) FROM prices WHERE idp=? LIMIT 1', (idp,))
    res = 365
    for last_date in c.fetchone():
        try:
...
def write_user_players(myid, username):
    ...
    c.execute('INSERT OR REPLACE INTO users (idu, name) VALUES (?, ?)', (myid,username))
    conn.commit()
    result = []
    for dato in info_user[6:]:
        player_name = dato[1].strip()
        idp = com.info_player_id(player_name)
        c.execute('INSERT OR REPLACE INTO players (idp, name, idu) VALUES (?,?,?)', (idp, player_name, myid))
        conn.commit()
...
def write_prices_player(player, idp=None):
    ...
    if days == 0:
        ...
        c.execute('SELECT p.idp,p.date,p.price FROM prices p WHERE p.idp=? AND p.date>=(SELECT t.date FROM transactions t WHERE t.idp=? AND t.type="Buy" ORDER BY t.date DESC LIMIT 1) ORDER BY p.date ASC', (idp,idp,))
        barras = c.fetchall()
        if len(barras) == 0:
            c.execute('SELECT p.idp,p.date,p.price FROM prices p WHERE p.idp=? ORDER BY p.date ASC', (idp,))
            barras = c.fetchall()
        return barras
    ...
    c.executemany('INSERT OR IGNORE INTO prices (idp,date,price) VALUES (?,?,?)', to_insert)
    conn.commit()
    # Devolvemos las barras desde la fecha de compra, si no lo hemos comprado devolvemos todas las barras
    c.execute('SELECT p.idp,p.date,p.price FROM prices p WHERE p.idp=? AND p.date>=(SELECT t.date FROM transactions t WHERE t.idp=? AND t.type="Buy" ORDER BY t.date DESC LIMIT 1) ORDER BY p.date ASC', (idp,idp,))
    barras = c.fetchall()
...
def write_new_player(player):
    ...
    '''
    idp = com.info_player_id(player)
    exists=c.execute('SELECT count(*) FROM players WHERE idp=? AND name=?', (idp, player)).fetchone()[0]
    if not exists:
        try:
            c.execute('INSERT OR IGNORE INTO players (idp,name) VALUES (?,?)', (idp, player))
            conn.commit()
        except:
...
def player_prices(name):
    ...
    headers = {"Content-type": "application/x-www-form-urlencoded","Accept": "text/plain",'Referer': url_comuniazo,"User-Agent": user_agent}
...
def check_buy(name, min_price, mkt_price):
    ...
    c.execute('SELECT pr.price,pr.date FROM prices pr,players pl WHERE pl.idp=pr.idp AND pl.name=? AND pr.date>? ORDER BY pr.date ASC', (name,from_date,) )
    maxH=0
    fecha='19700101'
