...
class MyThread(threading.Thread):
    ...
    def run(self):
        ...
        sql = '''insert into visitedCamp values (%s)''' % self.idCamp
        print sql
        c = self.db.cursor()
...
def tweetProds(idCamp, idCat, idLine, idProd, minDisc, maxDisc, t):
    b.go('''http://es.buyvip.com/ProductAction.bv?idCampaign=%s&idCategory=%s&idLine=%s&idProduct=%s''' % (
    idCamp, idCat, idLine, idProd))
    html = b.get_html()
    try:
        prodNoAvail = re.search(
            'SelectedWaitingList.bv\?idCampaign=([0-9]*)&idCategory=([0-9]*)&idLine=(-?[0-9]*)&idProduct=([0-9]*)',
            html).group(4)
    ...
    if dto > minDisc and dto < maxDisc:
        ...
        g = Googl()
        shortedurl = g.shorten_url(
            '''http://es.buyvip.com/ProductAction.bv?idCampaign=%s&idCategory=%s&idLine=%s&idProduct=%s''' % (
            idCamp, idCat, idLine, idProd))
        ...
        tweet = '''%s | %s | %.2fe/%.2fe | %.0f%%dto | %s | ''' % (
        nameBrand, nameProd, actualPrice, realPrice, dto, shortedurl)
        try:
            # t.PostUpdate(tweet)
